...
bindsym $mod+d exec --no-startup-id sh ~/.config/polybar/panels/scripts/launcher.sh --gnome
# The other mode search for applications in /usr/bin or /usr/sbin.
# This should only be used if a program lacks a .desktop file.
#bindsym $mod+d exec --no-startup-id rofi -show run -themes 
...
exec_always feh --bg-fill ~/.config/i3/599379.jpg
...
for_window [class="^.*"] border pixel 0 #F9FAF9
